...
The command requires Node.js, Bash, `jq`, `git`, either `lockf` or `flock`, and exactly `codex-cli 0.154.0` on `PATH`. Updating that pin requires reviewing the selected release's generated hook schemas and re-running every fixture. A separate live acceptance command is documented below.
...
The hook manages only projects that carry their workspace at the canonical root. It resolves `<cwd>/.millstrand` first and otherwise the Git common directory's repository root, so linked worktrees and subdirectories reach the same workspace. Without an explicit workspace, any other project returns before the configured-source probe, the OS lock, and Strand with no output at all, and a managed `native-v1` launch is never gated.
